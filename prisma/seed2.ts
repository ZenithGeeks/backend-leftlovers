...
async function main() {
    ...
    const ongAddress = await prisma.address.create({
        data: {
            id: ong.id,
            label: 'Ong Roti',
    ...
    const ongRoti = await prisma.merchant.create({
        data: {
            ownerUserId: ong.id,
            displayName: 'Roti Ong Halal Food',
            description: 'The best roti in town.',
            categoryId: ongCategory.id,
            addressId: ongAddress.id,
            status: MerchantStatus.APPROVED,
            listImageUrl: 'https://picsum.photos/400?bento',
            StoreImageUrl: 'https://picsum.photos/800?restaurant'
        }
    ...
    const ongMenuItem = await prisma.menuItem.create({
        data: {
            merchantId: ongRoti.id,
            name: 'Roti Burger',
            description: 'Roti dough is the bun.',
            basePrice: 159.0,
            originalPrice: 169.0,
            leftoverQty: 21,
    ...
    const ongOptionGroup = await prisma.optionGroup.create({
        data: {
            name: 'Add-ons',
            merchantId: ongRoti.id,
            minSelect: 0,
            maxSelect: 2,
            menu: {
                connect: { id: ongMenuItem.id }
            },
            options: {
                create: [
                    { name: 'Extra pickle', priceDelta: 10.0 },
                    { name: 'Extra meat', priceDelta: 20.0 }
                ]
            }
        },
